...
//Inicializando endereço chave pública do aquivo puzzles.txt // => Alterar aqui
...
function generate() {
    ...
    //console.log(ck.toString());   // => Private (WIF): 5HpHagT65TZzG1PH3CSu63k8DbpvD8s5ipCnYRNeQuRFKarWVVs: 1Pq6ZX74n7KvWpHtLX19rLnE37iMA4FTWC
        ...
        if (data.includes(public)){
            ...
            // Salvando a carteira e sua chave privada em um arquivo chamado Multi_Puzzles.txt.
            fs.appendFileSync('./Multi_Puzzles.txt', successString, (err) => {
            if (err) throw err; })
        }
    }
    //console.log(`${key}`);        // => Decimal (KEY): 30568377312064202855      
    process.title = "Multi-Puzzles Bitcoin" + " Decimal KEY: " + (` ${key} `) + 'Falta pesquisar: ' + (`${maxkey}` - `${key}`);
}
};
...
while(true){
    ...
    //Salva a carteira e sua chave após 1 minuto no arquivo Continue_Multi_Puzzles.txt na mesma pasta
    if(today.getSeconds()%60 === 0){
        var successString = "Decimal KEY: " + key + "\nData: " + currentDate + "  Hora: " + h + ":" + m + ":" + s + 0;
        fs.writeFileSync('./Continue_Multi_Puzzles.txt', successString, (err) => {
        if (err) throw err; })
    };
    ...
    if (key > maxkey){   
        ...
        console.log(`${successString}`)
        fs.appendFileSync('./Multi_Puzzles.txt', successString,)
